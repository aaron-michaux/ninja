...
bool CanonicalizePath(string* path, uint64_t* slash_bits, string* err)
{
   ...
   char* str  = 0;
   if(len > 0) str = &(*path)[0];
   if(!CanonicalizePath(str, &len, slash_bits, err)) return false;
   path->resize(len);
   return true;
}
                      ...
                      string* err)
{
   ...
   while(src < end) {
      if(*src == '.') {
         if(src + 1 == end || IsPathSeparator(src[1])) {
            ...
            continue;
                   ...
                   && (src + 2 == end || IsPathSeparator(src[2]))) {
      ...
      while(src != end && !IsPathSeparator(*src)) *dst++ = *src++;
...
void GetWin32EscapedString(const string& input, string* result)
{
   ...
   for(string::const_iterator it = input.begin(), end = input.end(); it != end;
       ++it) {
      switch(*it) {
      case kBackslash: ++consecutive_backslash_count; break;
      case kQuote:
         result->append(span_begin, it);
         ...
         break;
      default: consecutive_backslash_count = 0; break;
      }
   }
   result->append(span_begin, input.end());
...
int ReadFile(const string& path, string* contents, string* err)
{
   ...
   FILE* f = is_stdin ? stdin : fopen(path.c_str(), "rb");
   if(!f) {
...
void SetCloseOnExec(int fd)
{
   ...
   if(flags < 0) {
      ...
      if(fcntl(fd, F_SETFD, flags | FD_CLOEXEC) < 0) perror("fcntl(F_SETFD)");
   }
#else
   HANDLE hd = (HANDLE) _get_osfhandle(fd);
   if(!SetHandleInformation(hd, HANDLE_FLAG_INHERIT, 0)) {
      fprintf(
          stderr, "SetHandleInformation(): %s", GetLastErrorString().c_str());
   }
#endif // ! _WIN32
}
...
const char* SpellcheckString(const char* text, ...)
{
   ...
   const char* word;
   while((word = va_arg(ap, const char*))) words.push_back(word);
   va_end(ap);
...
string StripAnsiEscapeCodes(const string& in)
{
   ...
   for(size_t i = 0; i < in.size(); ++i) {
      ...
      while(i < in.size() && !islatinalpha(in[i])) ++i;
   }
   return stripped;
}
...
#ifdef _WIN32
   int fh = _sopen(
       path.c_str(), _O_RDWR | _O_CREAT, _SH_DENYNO, _S_IREAD | _S_IWRITE);
