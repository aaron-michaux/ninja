...
struct NinjaMain : public BuildLogUser
{
   ...
   bool CollectTargetsFromArgs(int argc,
                               char* argv[],
                               vector<Node*>* targets,
                               string* err);
   ...
   virtual bool IsPathDead(StringPiece s) const
   {
      ...
      // the deps log, but having a deps edge product an output thats input to
      ...
      if(mtime == -1) Error("%s", err.c_str()); // Log and ignore Stat() errors.
      return mtime == 0;
   }
};
...
void Usage(const BuildConfig& config)
{
   fprintf(
       ...
       "  -a       never strip ansi color codes\n"
       "\n"
...
bool NinjaMain::RebuildManifest(const char* input_file, string* err)
{
   ...
   if(!CanonicalizePath(&path, &slash_bits, err)) return false;
   Node* node = state_.LookupNode(path);
   if(!node) return false;

   Builder builder(&state_, config_, &build_log_, &deps_log_, &disk_interface_);
   if(!builder.AddTarget(node, err)) return false;

   if(builder.AlreadyUpToDate())
      return false; // Not an error, but we didn't rebuild.

   if(!builder.Build(err)) return false;
...
Node* NinjaMain::CollectTarget(const char* cpath, string* err)
{
   string path = cpath;
   uint64_t slash_bits;
   if(!CanonicalizePath(&path, &slash_bits, err)) return NULL;
...
bool NinjaMain::CollectTargetsFromArgs(int argc,
                                       char* argv[],
                                       vector<Node*>* targets,
                                       string* err)
{
...
int NinjaMain::ToolQuery(const Options* options, int argc, char* argv[])
{
   ...
   for(int i = 0; i < argc; ++i) {
      ...
      for(vector<Edge*>::const_iterator edge = node->out_edges().begin();
          edge != node->out_edges().end();
          ++edge) {
         for(vector<Node*>::iterator out = (*edge)->outputs_.begin();
             out != (*edge)->outputs_.end();
             ++out) {
            printf("    %s\n", (*out)->path().c_str());
         }
      }
...
int NinjaMain::ToolDeps(const Options* options, int argc, char** argv)
{
   ...
   if(argc == 0) {
      for(vector<Node*>::const_iterator ni = deps_log_.nodes().begin();
          ni != deps_log_.nodes().end();
          ++ni) {
         if(deps_log_.IsDepsEntryLiveFor(*ni)) nodes.push_back(*ni);
      }
   } else {
      string err;
      if(!CollectTargetsFromArgs(argc, argv, &nodes, &err)) {
...
{
   if(!edge) return;
   if(!seen->insert(edge).second) return;
   ...
   if(!edge->is_phony()) puts(edge->EvaluateCommand().c_str());
}
...
int NinjaMain::ToolCommands(const Options* options, int argc, char* argv[])
{
   ...
   while((opt = getopt(argc, argv, const_cast<char*>("hs"))) != -1) {
      switch(opt) {
      case 's': mode = PCM_Single; break;
      case 'h':
      default:
         printf("usage: ninja -t commands [options] [targets]\n"
                "\n"
                "options:\n"
                "  -s     only print the final command to build [target], not "
                "the whole chain\n");
         return 1;
      }
   }
...
int NinjaMain::ToolClean(const Options* options, int argc, char* argv[])
{
   ...
   while((opt = getopt(argc, argv, const_cast<char*>("hgr"))) != -1) {
      switch(opt) {
      case 'g': generator = true; break;
      case 'r': clean_rules = true; break;
      case 'h':
      default:
         printf("usage: ninja -t clean [options] [targets]\n"
                ...
                "  -r     interpret targets as a list of rules to clean "
                "instead\n");
         return 1;
      }
   }
...
int NinjaMain::ToolCompilationDatabase(const Options* options,
                                       int argc,
                                       char* argv[])
{
   bool first = true;
...
{
   if(!EnsureBuildDirExists()) return 1;
...
{
   // RLE encoded.
   const char* urtle = " 13 ,3;2!2;\n8 ,;<11!;\n5 `'<10!(2`'2!\n11 ,6;, `\\. "
                       "`\\9 .,c13$ec,.\n6 "
                       ",2;11!>; `. ,;!2> .e8$2\".2 \"?7$e.\n <:<8!'` 2.3,.2` "
                       ",3!' ;,(?7\";2!2'<"
                       "; `?6$PF ,;,\n2 `'4!8;<!3'`2 3! ;,`'2`2'3!;4!`2.`!;2 "
                       "3,2 .<!2'`).\n5 3`5"
   ...
   for(const char* p = urtle; *p; p++) {
      if('0' <= *p && *p <= '9') {
         ...
         for(int i = 0; i < max(count, 1); ++i) printf("%c", *p);
         count = 0;
      }
   }
   return 0;
...
bool DebugEnable(const string& name)
{
   if(name == "list") {
      ...
      const char* suggestion = SpellcheckString(name.c_str(),
                                                "stats",
                                                "explain",
                                                "keepdepfile",
                                                "keeprsp",
...
bool NinjaMain::OpenBuildLog(bool recompact_only)
{
   ...
   if(!build_dir_.empty()) log_path = build_dir_ + "/" + log_path;
   ...
   if(recompact_only) {
      ...
      if(!success) Error("failed recompaction: %s", err.c_str());
      return success;
   }
...
bool NinjaMain::OpenDepsLog(bool recompact_only)
{
   ...
   if(!build_dir_.empty()) path = build_dir_ + "/" + path;
...
int NinjaMain::RunBuild(int argc, char** argv)
{
   ...
   if(!builder.Build(&err)) {
      ...
      if(err.find("interrupted by user") != string::npos) { return 2; }
      return 1;
   }
...
int ReadFlags(int* argc, char*** argv, Options* options, BuildConfig* config)
{
   ...
   while(!options->tool
         && (opt = getopt_long(
                 *argc, *argv, "d:f:j:k:l:nt:aqvw:C:h", kLongOptions, NULL))
                != -1) {
      switch(opt) {
      case 'd':
         if(!DebugEnable(optarg)) return 1;
         break;
      case 'f': options->input_file = optarg; break;
      case 'j': {
         char* end;
         int value = strtol(optarg, &end, 10);
         if(*end != 0 || value <= 0) Fatal("invalid -j parameter");
      ...
      case 'k': {
         char* end;
         int value = strtol(optarg, &end, 10);
         if(*end != 0) Fatal("-k parameter not numeric; did you mean -k 0?");
      ...
      case 't':
         ...
         if(!options->tool) return 0;
         break;
      case 'v': config->verbosity = BuildConfig::VERBOSE; break;
      case 'a': config->strip_ansi = true; break;
      case 'q': config->verbosity = BuildConfig::QUIET; break;
      case 'w':
         if(!WarningEnable(optarg, options)) return 1;
         break;
      case 'C': options->working_dir = optarg; break;
      case OPT_VERSION: printf("%s\n", kNinjaVersion); return 0;
      case 'h':
      default: Usage(*config); return 1;
      }
   }
   *argv += optind;
   *argc -= optind;
...
int real_main(int argc, char** argv)
{
   ...
   for(int cycle = 1; cycle <= kCycleLimit; ++cycle) {
      ...
      if(!ninja.EnsureBuildDirExists()) return 1;

      if(!ninja.OpenBuildLog() || !ninja.OpenDepsLog()) return 1;
      ...
      if(g_metrics) ninja.DumpMetrics();
      return result;
   }
