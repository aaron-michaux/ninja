...
struct Plan
{
 ...
 private:
   ...
   /// or its dependents.  If an entry maps to false, we do not want to build
   /// it, but we might want to build one of its dependents.  If the entry maps
   /// to true, we want to build it.
   map<Edge*, bool> want_;
...
struct BuildStatus
{
 ...
 private:
   ...
   struct RateInfo
   {
      ...
      {
         if(edges && stopwatch_.Elapsed()) rate_ = edges / stopwatch_.Elapsed();
      }
   ...
   struct SlidingRateInfo
   {
      ...
      {
         if(update_hint == last_update_) return;
         last_update_ = update_hint;

         if(times_.size() == N) times_.pop();
