                                    ...
                                    string* err)
{
   ...
   if(!edge) {
      ...
      if(node->status_known()) return true;
      // This node has no in-edge; it is dirty if it is missing.
      if(!node->StatIfNecessary(disk_interface_, err)) return false;
      if(!node->exists())
   ...
   if(edge->mark_ == Edge::VisitDone) return true;

   // If we encountered this edge earlier in the call stack we have a cycle.
   if(!VerifyDAG(node, stack, err)) return false;
   ...
   for(vector<Node*>::iterator o = edge->outputs_.begin();
       o != edge->outputs_.end();
       ++o) {
      if(!(*o)->StatIfNecessary(disk_interface_, err)) return false;
   }
   ...
   for(vector<Node*>::iterator i = edge->inputs_.begin();
       ...
       ++i) {
      // Visit this input.
      if(!RecomputeDirty(*i, stack, err)) return false;

      // If an input is not ready, neither are our outputs.
      if(Edge* in_edge = (*i)->in_edge()) {
         if(!in_edge->outputs_ready_) edge->outputs_ready_ = false;
      }
      ...
      if(!edge->is_order_only(i - edge->inputs_.begin())) {
         ...
         if((*i)->dirty()) {
            ...
            if(!most_recent_input
               || (*i)->mtime() > most_recent_input->mtime()) {
               most_recent_input = *i;
            }
         }
      }
...
bool DependencyScan::VerifyDAG(Node* node, vector<Node*>* stack, string* err)
{
   ...
   if(edge->mark_ != Edge::VisitInStack) return true;

   // We have this edge earlier in the call stack.  Find it.
   vector<Node*>::iterator start = stack->begin();
   while(start != stack->end() && (*start)->in_edge() != edge) ++start;
                                          ...
                                          Node* output)
{
   ...
   if(build_log()) {
      ...
      if(entry || (entry = build_log()->LookupByOutput(output->path()))) {
         if(!generator
            && BuildLog::LogEntry::HashCommand(command)
                   != entry->command_hash) {
            // May also be dirty due to the command changing since the last
            // build. But if this is a generator rule, the command changing does
            // not make us dirty.
            EXPLAIN("command line changed for %s", output->path().c_str());
            return true;
         }
         if(most_recent_input && entry->mtime < most_recent_input->mtime()) {
            // May also be dirty due to the mtime in the log being older than
            // the mtime of the most recent input.  This can occur even when the
            // mtime on disk is newer if a previous run wrote to the output file
            // but exited with an error or was interrupted.
            EXPLAIN(
      ...
      if(!entry && !generator) {
         EXPLAIN("command line not found in log for %s",
                 output->path().c_str());
         return true;
      }
   }
...
bool Edge::AllInputsReady() const
{
   for(vector<Node*>::const_iterator i = inputs_.begin(); i != inputs_.end();
       ++i) {
      if((*i)->in_edge() && !(*i)->in_edge()->outputs_ready()) return false;
   }
   return true;
}
...
string EdgeEnv::LookupVariable(const string& var)
{
   ...
   if(recursive_) {
      ...
      if((it = find(lookups_.begin(), lookups_.end(), var)) != lookups_.end()) {
         string cycle;
         for(; it != lookups_.end(); ++it) cycle.append(*it + " -> ");
         cycle.append(var);
   ...
   if(recursive_ && eval) lookups_.push_back(var);
...
string Edge::EvaluateCommand(bool incl_rsp_file)
{
   ...
   if(incl_rsp_file) {
      ...
      if(!rspfile_content.empty()) command += ";rspfile=" + rspfile_content;
   }
   return command;
}
...
bool Edge::use_console() const { return pool() == &State::kConsolePool; }
...
bool ImplicitDepLoader::LoadDeps(Edge* edge, string* err)
{
   ...
   if(!deps_type.empty()) return LoadDepsFromLog(edge, err);
